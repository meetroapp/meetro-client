...
export async function prepareWorkCenterPolishFixture() {
  ...
  try {
    ...
    const Card = await compile(card, 'job,lifecycle,visual,scheduledDate,jobAlertCount,jobListPresentation,selectedWorkCenterJob,setSelectedJobDetailView,setIsJobHistoryMode,setSelectedWorkCenterAlertStage,setIsWorkCenterSectionOpen,setSelectedWorkCenterJob');
    ...
    const noop = () => {};
    const cardProps = { job: { id: 'job-1', customer: 'Liam Molina', title: 'Bathroom Exhaust Fan Replacement' }, lifecycle, visual: { location: 'Cape Coral, FL' }, scheduledDate: 'Not scheduled', jobAlertCount: 2, jobListPresentation: { statusLabel: liveJob.stage.label, nextStepLabel: 'Complete Job', responsibilityLabel: 'Professional' }, selectedWorkCenterJob: null, setSelectedJobDetailView: noop, setIsJobHistoryMode: noop, setSelectedWorkCenterAlertStage: noop, setIsWorkCenterSectionOpen: noop, setSelectedWorkCenterJob: noop };
