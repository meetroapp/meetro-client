...
test("primary Work Center card counts and destinations remain unchanged", () => {
  ...
  assert.match(cards.revenue, /totalJobRevenue/);
...
test("primary Work Center cards do not expose implementation terminology", () => {
  const primaryCards = dashboardSource.slice(
    dashboardSource.indexOf("const workCenterPrimaryNavigationCards = ["),
    dashboardSource.indexOf("const workCenterLandingAlert", dashboardSource.indexOf("const workCenterPrimaryNavigationCards = ["))
  );
  assert.doesNotMatch(
    primaryCards,
    /canonical|legacy|browser-stored|compatibility|payment authority|read-only/i
  );
});
