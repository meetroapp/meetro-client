...
import {
  ...
  matchesServiceArea,
} from "../utils/serviceAreaMatching.js";

test("backend matches the same zip using current fallback fields", () => {
  assert.equal(
    matchesServiceArea(
...
test("backend matches the same city using current fallback fields", () => {
...
test("backend blocks different city and zip fallback locations", () => {
  const summary = getServiceAreaMatchSummary(
...
test("backend matches coordinates within service radius", () => {
  const summary = getServiceAreaMatchSummary(
    {
      lat: 26.5629,
      lng: -81.9495,
...
test("backend blocks coordinates outside service radius", () => {
  const summary = getServiceAreaMatchSummary(
    {
      latitude: 26.5629,
      longitude: -81.9495,
...
test("backend missing location fails closed for marketplace exposure", () => {
  const summary = getServiceAreaMatchSummary(
...
test("backend explicit local demo safe records can bypass missing location", () => {
  const summary = getServiceAreaMatchSummary(
...
test("backend distance calculation is deterministic and uses miles", () => {
