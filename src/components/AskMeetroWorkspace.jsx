...
const ASK_MEETRO_COMPOSER_MIN_HEIGHT = 44;
const ASK_MEETRO_COMPOSER_MAX_HEIGHT = 144;

function syncAskMeetroComposerHeight(element) {
  if (!element) return;
  ...
  const contentHeight = Math.max(
    ...
    ASK_MEETRO_COMPOSER_MIN_HEIGHT
  );

  const height = Math.min(
    contentHeight,
    ASK_MEETRO_COMPOSER_MAX_HEIGHT
  );

  element.style.height = `${height}px`;
  element.style.overflowY =
    contentHeight > ASK_MEETRO_COMPOSER_MAX_HEIGHT ? "auto" : "hidden";
...
export default function AskMeetroWorkspace({ context = {}, role = "personal", initialQuestion = "", currentPage = "home", onClose, setPage, session, onSessionChange, resolveActions = resolveAskMeetroActions, requestConversation, completionApi = { review: reviewAskMeetroCompletion, apply: applyAskMeetroCompletion } }) {
  ...
  async function send(event) {
    ...
    // A new typed message is a new operation, not a retry of a prior
    // ambiguity selection.
    ...
    try {
      ...
      setReview(null);
      setInput((current) =>
        current.trim() === text && !retainComposer
          ? ""
          : current
      );
    } catch (failure) {
  ...
  async function chooseResolvedRecord(index) {
    if (
      inFlight.current ||
      resolution?.status !== "AMBIGUOUS" ||
      !resolution.continuation?.reference ||
      !Number.isInteger(index) ||
      index < 0 ||
      index >= resolution.records.length
    ) return;

    const selected = resolution.records[index];
    const instruction = String(resolution.instruction || "").trim();

    if (!instruction) {
      selectionAttempt.current = null;
      setError("The prior Ask Meetro question is unavailable. Search again.");
      setResolution(null);
      return;
    }

    const selectionSignature = JSON.stringify({
      reference: resolution.continuation.reference,
      index,
      instruction,
      context,
    });

    if (selectionAttempt.current?.signature !== selectionSignature) {
    ...
    try {
      requestController.current = new AbortController();
      const result = await resolveAskMeetroRequest(instruction, {
        context,
        role,
        setPage,
        messages,
        ...
        get idempotencyKey() {
          return selectionAttempt.current.key ||= createIntelligenceKey();
        },
      });
      ...
      setMessages((current) => [
        ...current,
        { role: "user", text: `Use ${selected.label || "this record"}.` },
        { role: "assistant", text: result.text },
      ]);
      setResolution(result.resolution ? { ...result.resolution, instruction } : null);
      setActions((result.actions || []).filter((action) => action.status === "PROPOSED" && action.route));
      setReview(null);
    } catch (failure) {
      if (
        /^INTELLIGENCE_(?:PROVIDER_|RESULT_|OPERATION_FAILED_REPLAY|OPERATION_CONFLICT)/.test(
          failure?.code || ""
        )
      ) {
        selectionAttempt.current = null;
      ...
      if (mounted.current) {
        setError(failure?.message || "That record selection could not be verified. Search again.");
      }
    } finally {
  ...
  return <div className={`ask-meetro-shell${viewport?.keyboard ? " is-keyboard-open" : ""}`} style={viewport ? { height: viewport.height, top: viewport.top } : undefined}>
    <main className="ask-meetro-workspace" aria-labelledby="ask-meetro-title">
      ...
      <div className="ask-meetro-conversation" role="region" aria-label="Ask Meetro conversation">
        ...
        {resolution?.reviewRequired === true && resolution?.status === "RESOLVED" && !review ? <section className="ask-meetro-resolution-review" aria-label="Resolved record requires Review"><span>Resolved · not applied</span><h2>Review required</h2><p>Meetro verified the target record. Continue through its existing governed workflow before anything can change.</p>{resolution.records?.[0]?.label ? <strong>{resolution.records[0].label}</strong> : null}{actions.length ? <small>The governed Review action is available below.</small> : <small>No safe direct Review route is available from this screen. Nothing has been changed.</small>}</section> : null}
      ...
      <footer className="ask-meetro-footer">
        ...
        <form className="ask-meetro-composer" onSubmit={send}><input ref={attachRef} type="file" multiple hidden onChange={attach} /><input ref={photoRef} type="file" accept="image/*" capture="environment" multiple hidden onChange={attach} /><button type="button" aria-label="Attach" onClick={() => attachRef.current?.click()}><MeetroIcon name="addProject" size={20} decorative /></button><button type="button" aria-label="Photo" onClick={() => photoRef.current?.click()}><MeetroIcon name="portfolio" size={20} decorative /></button><textarea
          ref={(element) => {
            composerRef.current = element;
            syncAskMeetroComposerHeight(element);
          }}
          aria-label="Ask Meetro anything"
          placeholder="Ask Meetro anything..."
          value={input}
          maxLength={5000}
          rows={1}
          onChange={(event) => {
            setInput(event.target.value);
            syncAskMeetroComposerHeight(event.currentTarget);
          }}
        /><button type="button" aria-label={voiceState !== "idle" ? "Stop voice" : "Voice"} aria-pressed={listening} onClick={voice}><MeetroIcon name="microphone" size={20} decorative /></button><button type="submit" className="ask-meetro-send" aria-label="Send" disabled={!input.trim() || busy}><MeetroIcon name="publishProject" size={20} decorative /></button></form>
