...
export function PreWorkPlanPresentation({ approvedWork, preparation, approvedVisits, readiness, record = {}, language = "en", canonicalRecord = {}, jobId, setPage, onCanonicalChange, preparationInitialOpen = "", showManageControls = true, focusVisitId = "", stageSeparated = false }) {
  return <div style={styles.preWork} data-work-plan-review-state={preparationInitialOpen || "collapsed"}>
    ...
    {!stageSeparated && <CompactArea id="work-schedule" title="Work Schedule" summary="When the work is planned." icon="schedule"><CompactSchedule visits={approvedVisits} record={record} language={language} canonicalRecord={canonicalRecord} setPage={setPage} focusVisitId={focusVisitId} /></CompactArea>}
    <CompactArea id="work-readiness" title="Ready to Start" summary="Everything is in place." icon="completion">
      <div style={styles.readinessGrid}><ReadinessCheck complete={readiness.approvedScope} label="Customer approved" />{!stageSeparated && <ReadinessCheck complete={readiness.depositSatisfied} label="Deposit received" />}<ReadinessCheck complete={readiness.preparationReady} label="Materials ready" />{!stageSeparated && <ReadinessCheck complete={readiness.scheduled} label="Work scheduled" />}</div>
...
export default function ProfessionalWorkPlanWorkspace({ jobId, record = {}, preferredQuoteId = "", focusVisitId = "", liveJob = null, language = "en", setPage, onCanonicalChange, stageSeparated = false }) {
  ...
  return (
    <section className="professional-work-plan" style={styles.section} aria-label={title} data-work-plan-primary-container="true" data-work-plan-status={state.status} data-work-plan-error={state.errors.join(",")} data-work-plan-job-id={jobId || ""} data-work-execution-mode={mode}>
      ...
      {state.status === "ready" && mode === "PRE_WORK" && <PreWorkPlanPresentation approvedWork={approvedWork} preparation={state.preparation} approvedVisits={approvedVisits} readiness={readiness} record={record} language={language} canonicalRecord={canonicalRecord} jobId={jobId} setPage={setPage} onCanonicalChange={onCanonicalChange} focusVisitId={focusVisitId} stageSeparated={stageSeparated} />}
      ...
      {state.status === "ready" && mode !== "PRE_WORK" && <>
        ...
        <div style={styles.referenceGrid}><div><strong>Preparation</strong><span>{readable(executionView.preparation?.preparationState)}</span></div>{!stageSeparated && <div><strong>Schedule</strong><span>{executionView.schedule.length ? `${executionView.schedule.length} visit${executionView.schedule.length === 1 ? "" : "s"}` : "Unavailable"}</span></div>}</div>
