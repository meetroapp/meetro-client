...
export function authorityKeys(value, keys) {
  return [...keys, ...['authority', 'sourceType', 'sourceLabel'].filter(key => Object.hasOwn(value || {}, key) && !keys.includes(key))];
}
export function validJobAuthority(value) {
  if (value?.sourceType === 'emergency_request') return value.sourceLabel === 'Emergency' && value.requestId === null && positive(value.relationshipId) && !value.authority;
...
export function jobAuthorityFields(value) {
  return { ...(value.authority ? {authority:Object.freeze({...value.authority})} : {}),
    ...Object.fromEntries(['sourceType', 'sourceLabel'].filter(key => Object.hasOwn(value || {}, key)).map(key => [key, value[key]])) };
}
