  ...
  function generateMaterialsSuggestion() {
    ...
    if (!description) {
      setMaterialsAiSuggestion(
        activeLanguage === "es"
          ? "Describe el trabajo o los materiales que faltan para que AI pueda sugerir una lista."
          : "Describe the job or missing materials so AI can suggest a list."
      );
      return;
    }
    ...
    setMaterialsAiSuggestion(
      activeLanguage === "es"
        ? `Sugerencia AI para: ${description}\n• Material principal\n• Piezas de conexión\n• Sellador o cinta según el trabajo\n• Tornillos/herrajes si aplica\n• Material de respaldo por si hay desgaste oculto`
        : `AI suggestion for: ${description}\n• Main replacement material\n• Connection fittings\n• Sealant or tape as needed\n• Screws/hardware if applicable\n• Backup material in case hidden wear is found`
    );
  }

  function saveMaterialItem() {
    const currentMaterials = JSON.parse(
      localStorage.getItem("meetro_work_materials") || "[]"
    );
...
setPage("emergencyDispatch");
      ...
      <div style={workTabs}>
        ...
        <button
          style={activeTab === "materials" ? workTabActive : workTab}
                        ...
                        <>
                          <button
                            ...
                            onClick={() => {
                              ...
                              localStorage.setItem("activeWorkType", "scheduled");
                              localStorage.setItem("activeWorkSource", item.source || "schedule");
                              localStorage.setItem("activeWorkService", item.title || "Scheduled Visit");
                              localStorage.setItem("activeWorkLocation", item.location || "");
                              localStorage.setItem("activeWorkConversationId", item.conversationId || "");
                              localStorage.setItem("activeWorkScheduleId", item.id || "");

                              localStorage.setItem("activeJobService", item.title || "Scheduled Visit");
                              localStorage.setItem("activeJobCustomer", item.title || "Customer");
                              localStorage.setItem("activeJobLocation", item.location || "");
                          ...
                          >
                            {activeLanguage === "es" ? "Iniciar trabajo" : "Start Job"}
                          </button>
              ...
              {universalActiveWork.status &&
                ...
                <div style={activeJobPanel}>
                    ...
                    <div>
                      ...
                      <div style={jobActivityNote}>
                        ...
                        {universalActiveWork.stage === "pausedMaterials" &&
                          (activeLanguage === "es"
                            ? "⏸ Trabajo pausado — esperando materiales"
                            : "⏸ Job paused — waiting on materials")}
                      </div>
                    </div>
                  ...
                  <div style={stageActions}>
                    ...
                    >
                      🚗 {activeLanguage === "es" ? "En camino" : "On the way"}
                    </button>
                    ...
                    >
                      📍 {activeLanguage === "es" ? "Llegué" : "Arrived"}
                    </button>
                    ...
                    >
                      🛠️ {activeLanguage === "es" ? "Trabajando" : "Working"}
                    </button>
                    ...
                    >
                      ⏸ {activeLanguage === "es" ? "Pausar por materiales" : "Pause for Materials"}
                    </button>
                  </div>
                  ...
                  <div style={jobActions}>
                    ...
                    <button
                      ...
                      onClick={() => {
                        localStorage.setItem(
                          "completionService",
                          universalActiveWork.service || "Scheduled Work"
                        );
                        localStorage.setItem(
                          "completionLocation",
                          universalActiveWork.location || ""
      ...
      {activeTab === "materials" && (
        <div style={section}>
          ...
          <div style={emptyCard}>
            ...
            <p style={emptyText}>
              {activeLanguage === "es"
                ? "Solicita materiales, pausa trabajos y usa AI para generar listas inteligentes."
                : "Request materials, pause jobs, and use AI to generate smart material lists."}
            </p>
            ...
            <textarea
              ...
              placeholder={
                activeLanguage === "es"
                  ? "Describe el trabajo o materiales faltantes..."
                  : "Describe the job or missing materials..."
              }
            />
                ...
                <div style={materialsListPanel}>
                  <div style={materialsListHeader}>
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (!conversationId || materials.length === 0) return;
                  ...
                  {materials.length === 0 ? (
                    <div style={materialsEmptyBox}>
                      {activeLanguage === "es"
                        ? "Aún no has guardado materiales."
                        : "No materials saved yet."}
                    </div>
                  ) : (
                    <div style={materialsRows}>
                      {materials.map((item) => (
                        <div style={materialRow} key={item.id}>
                          <div style={materialIcon}>
                            {String(item.title).toLowerCase().includes("pipe")
                              ? "🔩"
                              : String(item.title).toLowerCase().includes("cement")
                              ? "🧴"
                              : String(item.title).toLowerCase().includes("tape")
                              ? "🌀"
                              : "📦"}
                          </div>
                  ...
                  {materials.length > 0 && neededCount === 0 && (
                    <button
                      style={resumeReadyButton}
                      onClick={() => {
                        localStorage.setItem("activeWorkStage", "working");
                        localStorage.removeItem("activeWorkPauseReason");
                        setActiveTab("active");
                    ...
                    >
                      ▶️ {activeLanguage === "es"
                        ? "Todos los materiales listos — reanudar"
...
const activeStageButton = {
  ...
  color: "white",
  border: "1px solid #5b3df5",
  boxShadow: "0 10px 24px rgba(91,61,245,.25)",
};
